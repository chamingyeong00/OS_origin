...
public class LfuCore implements CorePolicy {
    ...
    public Page.STATUS operate(char data) {
        ...
        if (newPage.status == Page.STATUS.MIGRATION) {
            System.out.println("  → MIGRATION occurred. Evicted page with lowest frequency.");
        }

        System.out.println("Current Frame State:");
        for (Page p : frame_window) {
            int freq = frequencyMap.getOrDefault(p.data, 0);
            System.out.printf("  [PID=%d] %c | Freq=%d\n", p.pid, p.data, freq);
        }
